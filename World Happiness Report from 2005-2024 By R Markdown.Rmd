  ...
  World_happiness_data_2024 <- read.csv("C:/Users/m.alshannaq/Downloads/archive (7)/World-happiness-report-2024.csv") # nolint
  World_happiness_data <- read.csv("C:/Users/m.alshannaq/Downloads/archive (7)/World-happiness-report-updated_2024.csv") # nolint
  ...
  # Read the data
  World_happiness_data <- read.csv("C:/Users/m.alshannaq/Downloads/archive (7)/World-happiness-report-updated_2024.csv")
  ...
  write.csv(world_happiness_clean_final, "world_happiness_clean_main.csv")
  ...
  # I am gonna repeat the process but for the World_happiness_data_2024 without the grouping
  ```{r}
  library(dplyr)
  library(lsa)
  ...
  write.csv(world_happiness_clean_2024, "world_happiness_clean_2024.csv")
  ```
  ...
  world_happiness_clean_2024 <- read.csv("C:/Users/m.alshannaq/Downloads/archive (7)/world_happiness_clean_2024.csv") # nolint
  head(world_happiness_clean_2024)

  p <- ggplot(world_happiness_clean_2024, aes(x = Regional.indicator, fill = Regional.indicator)) + # nolint
    geom_bar() +
    ...
    theme(legend.title = element_blank(),
          axis.text.x = element_text(angle = 90, hjust = 1)) # nolint
  ...
  colors <- brewer.pal(n = length(unique(world_happiness_clean_2024$Regional.indicator)), name = "Set3") # nolint

  ggplot(world_happiness_clean_2024, aes(x = Regional.indicator, y = Ladder.score, fill = Regional.indicator)) + # nolint
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 12),  #nolint
  ...
  # Now let's see the correlation between all the factors by Drawing a Correlation Matrix (Heatmap)
  ```{r}
  ...
  world_happiness_clean_main <- read.csv("world_happiness_clean_main.csv")
  numeric_columns <- select_if(world_happiness_clean_main, is.numeric) %>% select(-year) # nolint


  correlation_matrix <- cor(numeric_columns, use = "complete.obs")

  upper_triangle_mask <- matrix(NA, nrow = nrow(correlation_matrix), ncol = ncol(correlation_matrix))# nolint
  ...
  cor_life_ladder <- correlations["Life.Ladder", -1]  # nolint
  ...
  p <- ggplot(top_countries, aes(x = reorder(Country.name, Total_Life_Ladder), y = Total_Life_Ladder, fill = Category)) + #nolint
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(title = "Top 3 Happiest and Unhappiest Countries (Total Life Ladder Score)", #nolint
        x = "Country",
